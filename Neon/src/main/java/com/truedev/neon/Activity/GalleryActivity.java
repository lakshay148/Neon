...
import com.truedev.neon.Adapters.ImagesFoldersAdapter;
...
public class GalleryActivity extends BaseActivityGallery implements AdapterView.OnItemClickListener {
    ...
    ArrayList<ImageModel> previouslySelectedImages;
...
//            intent.putExtra(GALLERY_DELETED_PHOTOS, data.getExtras().getSerializable(FolderFiles.DELETED_PHOTOS);
        ...
        } else if (resultCode == FolderFiles.RESULT_DONT_SKIP_FOLDERS) {
            previouslySelectedImages= (ArrayList<ImageModel>) data.getExtras().getSerializable(FolderFiles.SELECTED_FILES);
        }
...
//                .withMaxSize(300, 400)
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_done) {
            Intent intent=new Intent();
            intent.putExtra(GALLERY_SELECTED_PHOTOS,previouslySelectedImages);
            setResult(RESULT_OK, intent);
            finish();
            return true;
        }
...
//        Intent intent = new Intent(this, GalleryContentProviderActivity.class);
        ...
        intent.putExtra(ALREADY_SELECTED_FILES , previouslySelectedImages);
    ...
    public boolean checkIfAlreadyPresent(FileInfo fileInfo) {
        if (previouslySelectedImages!= null && previouslySelectedImages.size() > 0) {
